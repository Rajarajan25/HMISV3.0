...
export default function Signup() {
  ...
  const handleChange = (values) => {
    ...
    addSite({
      variables: {
        site: {
          DefaultLanguageId: userLanguage,
          status: true,
          DisplayOrder: 1,
        },
      },
      ...
      let site_id = res.data.addSite._id;
      localStorage.setItem("site_id", site_id);
      onAddPassword(site_id, values);
    });
  };
  const onAddPassword = (site_id, values) => {
    addPassword({
      variables: {
        password: {
      ...
      let password_id = res.data.addPassword._id;
      onAddUser(site_id, values, password_id);
    });
  };
  const onAddUser = (site_id, values, password_id) => {
    addUser({
      variables: {
        userIn: {
          email: values.email,
      ...
      onPassWordUpdate(user_id, password_id);
      user.push(res.data.addUser);
      setUser({
        type: "SET_STATE_USER",
        payload: user,
      });
      onUpdateUser(res.data.addUser);
      localStorage.setItem("UserId", user_id);
      window.location.href = "/auth/personalinfo";
    });
  };
  const onPassWordUpdate = (user_id, password_id) => {
    updatePassword({
      variables: {
                ...
                {({ errors, handleSubmit }) => (
                  ...
                  >
                    ...
                    <div className="form-group reg_icon text-center align-items-center">
                      ...
                      >
                        <img
                          src="/media/auth-screen/google.svg"
                          alt="Goolge Icon"
                      ...
                      >
                        <img
                          src="/media/auth-screen/facebook.svg"
                          alt="facebook Icon"
                          className="socialIcon"
                        />
                      </button>
                      ...
                      >
                        <img
                          src="/media/auth-screen/twitter.svg"
                          alt="facebook Icon"
                          className="socialIcon"
                        />
                      </button>
