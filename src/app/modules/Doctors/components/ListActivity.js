...
export function ListActivity01() {  
  const [staff, setstaff] = React.useContext(StaffContext);
  const { staffs, loading, loadMore, hasNextPage } = usestaffs()
  const loadMoreStaffs = loading ? () => { } : loadMore
  const isPersonLoaded = index => !hasNextPage || index < staffs.length
  var healthy = [];
  var onfield = [];
  var labass = [];
  var emergency = [];
  if (staffs !== undefined && (staffs.length !== undefined || staffs.length > 0)) {
    staffs.map(value => {
      if (value !== null) {
                    ...
                    <div className="topMiddlecontent">
                      ...
                      {healthy.map(value => {
                            return (
                              <ul>
                              <>
                              <li><a href="javascript:void(0);" className="userLogoicon" data-toggle="modal" data-target="#commonModalright"><span className="listprofileIcon customProfileBG2">SK</span> <span>{value.user_name}</span></a></li>
